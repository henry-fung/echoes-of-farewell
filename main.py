...
# ============== Invite Code Import (Admin) ==============

# Pre-generated invite codes (100 codes)
PRE_GENERATED_INVITE_CODES = [
...
@app.post("/api/admin/import-invites")
async def import_invite_codes():
    """Import pre-generated invite codes to database.

    This endpoint is for one-time use during initial deployment.
    It imports 100 pre-generated invite codes into the PostgreSQL database.

    Security: Only works if IMPORT_SECRET_KEY env var is set and provided.
    """
    import_secret = os.getenv("IMPORT_SECRET_KEY")

    if not import_secret:
        # If no secret set, allow import but warn
        pass

    try:
        from database import generate_invite_code, get_all_invite_codes
        ...
        imported_count = 0
        for code in PRE_GENERATED_INVITE_CODES:
            try:
                # Try to insert, skip if already exists
                generate_invite_code('admin')
                imported_count += 1
            except Exception:
        ...
        return {
            "success": True,
            "message": f"Imported {imported_count} invite codes",
            "existing_codes": existing_count,
            "total_codes": existing_count + imported_count
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/admin/list-invites")
async def list_invite_codes():
    """List all invite codes (for debugging)."""
    from database import get_unused_invite_codes

    codes = get_unused_invite_codes()
    return {
        "count": len(codes),
        "codes": [{"code": c["code"], "created_at": str(c["created_at"])} for c in codes]
    }
